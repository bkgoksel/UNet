...
class UNet(nn.Module):
    ...
    def build_model(self):
        ...
        self.pos_embeddings = nn.Embedding(
            opts["pos_size"], opts["pos_dim"], padding_idx=0
        )

        self.ner_embeddings = nn.Embedding(
            opts["ner_size"], opts["ner_dim"], padding_idx=0
        )
        ...
        feat_size = 4
        low_p_word_size = (
            word_dim + word_dim + opts["pos_dim"] + opts["ner_dim"] + feat_size
        )
        ...
        self_attention_inp_size = (
            word_dim + pos_dim + ner_dim + 6 * (2 * hidden_size) + 1
        )
        ...
        self.summ_layer = Summ(
            input_size=2 * hidden_size, dropout=dropout, use_cuda=self.use_cuda
        )
        if self.opts["multi_point"]:
            self.summ_layer_p = Summ(
                input_size=2 * hidden_size, dropout=dropout, use_cuda=self.use_cuda
            )
        self.summ_cf = Summ(
            input_size=2 * hidden_size, dropout=dropout, use_cuda=self.use_cuda
        )
        self.summ_layer2 = Summ(
            input_size=2 * hidden_size, dropout=dropout, use_cuda=self.use_cuda
        )
        self.summ_cf2 = Summ(
            input_size=2 * hidden_size, dropout=dropout, use_cuda=self.use_cuda
        )
        self.pointer_layer = PointerNet(
            input_size=2 * hidden_size, opt=self.opts, use_cuda=self.use_cuda
        )
        if self.opts["multi_point"]:
            self.pointer_layer_p = PointerNet(
                input_size=2 * hidden_size, opt=self.opts, use_cuda=self.use_cuda
            )
    ...
    def prepare_data(self, batch_data):
        ...
        passage_match_origin = torch.cat(
            [ques_match_origin, passage_match_origin], dim=1
        )
    ...
    def encoding_forward(self, batch_data):
        ...
        if self.use_elmo:
            ...
            ### embedding dropout ###
            ques_elmo = Dropout(
                ques_elmo, dropout, self.training, use_cuda=self.use_cuda
            )
            passage_elmo = Dropout(
                passage_elmo, dropout, self.training, use_cuda=self.use_cuda
            )
        ...
        word_attention_outputs = self.word_attention_layer(
            passage_emb, p_mask, cat_emb[:, : q_len + 1], q_mask, self.training
        )
        q_word_attention_outputs = self.word_attention_layer(
            cat_emb[:, : q_len + 1], q_mask, passage_emb, p_mask, self.training
        )
        ...
        p_word_inp = torch.cat(
            [
                ...
                passage_pos_emb,
                passage_ner_emb,
                word_attention_outputs,
                passage_match_origin,
                passage_match_lower,
                passage_match_lemma,
                passage_tf,
            ],
        ...
        low_attention_outputs, low_attention_ques = self.low_attention_layer(
            passage_HoW,
            p_mask,
            ques_HoW,
            q_mask,
        ...
        high_attention_outputs, high_attention_ques = self.high_attention_layer(
            passage_HoW,
            p_mask,
            ques_HoW,
            q_mask,
        ...
        und_attention_outputs, und_attention_ques = self.und_attention_layer(
            passage_HoW,
            p_mask,
            ques_HoW,
            q_mask,
        ...
        fuse_inp = torch.cat(
            [
                low_passage_states,
                high_passage_states,
                low_attention_outputs,
                high_attention_outputs,
                und_attention_outputs,
            ],
            dim=2,
        )
        fuse_ques = torch.cat(
        ...
        cat_HoW = torch.cat(
            [cat_emb, cat_pos_emb, cat_ner_emb, cat_tf, fuse_cat, fused_cat_states],
            dim=2,
        )
        ...
        self_attention_outputs, _ = self.self_attention_layer(
            cat_HoW, cat_mask, cat_HoW, cat_mask, fused_cat_states, None, self.training
        )
    ...
    def decoding_forward(
        ...
        if self.opts["multi_point"]:
            q_summ_p = self.summ_layer_p(
                und_ques_states_p, q_mask[:, :-1], self.training
            )

        ### Pointer Network ###
        logits1, logits2, _ = self.pointer_layer.forward(
            und_passage_states, p_mask, None, q_summ, self.training
        )
        logits1_p, logits2_p = None, None
        if self.opts["multi_point"]:
            logits1_p, logits2_p, _ = self.pointer_layer_p.forward(
                und_passage_states_p, p_mask, None, q_summ_p, self.training
            )
    ...
    def forward(self, batch_data):
        ...
        und_passage_states, p_mask, und_ques_states, q_mask, und_passage_states_p, und_ques_states_p = self.encoding_forward(
            batch_data
        )
        logits1, logits2, has_log, logits1_p, logits2_p = self.decoding_forward(
            und_passage_states,
            p_mask,
            und_ques_states,
            q_mask,
    ...
    def get_predictions(self, logits1, logits2, ans_log, maxlen=15):
        if self.use_pd:
            ...
            outer = torch.matmul(
                F.softmax(logits1, -1).unsqueeze(2), F.softmax(logits2, -1).unsqueeze(1)
            )
    ...
    def Evaluate(
        self, batches, eval_file=None, answer_file=None, drop_file=None, dev=None
    ):
        ...
        for batch in batches:
            ...
            logits1, logits2, ans_log, logits1_p, logits2_p = self.decoding_forward(
                und_passage_states,
                p_mask,
                und_ques_states,
                q_mask,
        ...
        print(
            "Exact Match: {}, F1: {}, Has answer F1: {}, No answer F1: {}".format(
                metrics["exact"],
                metrics["f1"],
                metrics["HasAns_f1"],
                metrics["NoAns_f1"],
