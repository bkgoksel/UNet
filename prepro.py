        ...
        for article in tqdm(source["data"]):
            for para in article["paragraphs"]:
                ...
                for qa in para["qas"]:
                    ...
                    ques_tf = [
                        float(counter_[w]) / float(tf_total) for w in ques_lower_tokens
                    ]
                    ...
                    match_lemma = [
                        ...
                        for (c_lemma, c_lower) in zip(
                            context_lemmas, context_lower_tokens
                        )
                    ]
                    ...
                    context_lemma = {
                        lemma if lemma != "-PRON-" else lower
                        for lemma, lower in zip(context_lemmas, context_lower_tokens)
                    }
                    ques_match_origin = [w in context_tokens_set for w in ques_tokens]
                    ques_match_lower = [
                        w in context_lower_tokens_set for w in ques_lower_tokens
                    ]
                    ques_match_lemma = [
                        (q_lemma if q_lemma != "-PRON-" else q_lower) in context_lemma
                        for (q_lemma, q_lower) in zip(ques_lemmas, ques_lower_tokens)
                    ]
                    ...
                    example = {
                        ...
                        "ques_pos": ques_tags,
                        "ques_ner": ques_ents,
                        "ques_match_origin": ques_match_origin,
                        "ques_match_lower": ques_match_lower,
                        "ques_match_lemma": ques_match_lemma,
...
def get_embedding(
    ...
    NA = "NoAnswer"
    token2idx_dict = (
        {token: idx for idx, token in enumerate(embedding_dict.keys(), 3)}
        if token2idx_dict is None
        else token2idx_dict
    )
...
if __name__ == "__main__":
    ...
    char_size = 94
    char2id = None
    char_emb, char2id = get_embedding(
        char_counter,
        "char",
        size=char_size,
